            ...
            <div className="md:p-2 flex flex-col">
                ...
                <div className="flex flex-col space-y-2 w-11/12 md:w-3/4">
                    {responses.map((response, index) =>
                        <Response key={index} index={index} label={response.label} {...response}
                            onChange={(event, index) => handleResponse(event, index)}
                        />
                    )}
                    {/* <input className="border w-full p-3  outline-none rounded focus:border-black"
                        placeholder="Response for Yes" />
                    <input className="border w-full p-3  outline-none rounded focus:border-black"
                        placeholder="Response for No" /> */}
                </div>
            </div>
        </div>
    )
